...
#			print_r($obj); exit;
			...
			$type = $obj->notificationData->type?trim($obj->notificationData->type):'';
			...
			$detailed_description = $obj->notificationData->detailed_description?trim(addslashes($obj->notificationData->detailed_description)):'';
                        ...
                        if(!empty($title) && !empty($detailed_description) && !empty($canids) && !empty($type)){
                                $data = pushNotification($title, $short_description, $detailed_description, $canids, $type);
                        }else{
                                $data = array("status"=>"failure","response"=>array(), "message"=>"Invalid request format");
                        }
                break;
...
#                       print_r($obj); exit;
		...
		case 'sendPaymentLinkForAirmesh':
			$param['can_id'] = $obj->canId?trim($obj->canId):'';
			$param['cust_name'] = $obj->custName?trim($obj->custName):'';
			$param['industry_segment'] = $obj->industrySegment?trim($obj->industrySegment):'';
			$param['product_segment'] = $obj->productSegment?trim($obj->productSegment):'';
			$param['sr_number'] = $obj->srNumber?trim($obj->srNumber):'';
			$param['sr_creation_date'] = $obj->srCreationDate?trim($obj->srCreationDate):'';
                        $param['topup_id'] = $obj->topupId?trim($obj->topupId):'';
                        $param['topup_mode'] = $obj->topupMode?trim($obj->topupMode):'';
			$param['topup_amount'] = $obj->topupAmount?trim($obj->topupAmount):'';
			$param['router_detail'] = $obj->routerDetail?trim($obj->routerDetail):'';
			$param['created_by'] = $obj->createdBy?trim($obj->createdBy):'';
                        $param['mobile'] = $obj->mobile?trim($obj->mobile):'';
			$param['email'] = $obj->email?trim($obj->email):'';
			$param['inst_city'] = $obj->installationCity?trim($obj->installationCity):'';
			$data = sendPaymentLinkForAirmesh($param);	
			
		 break;		
/********** End ***********/

/********** Start PG SDK Integration API ********/
		case 'createTransaction':
#                       print_r($obj); exit;
			$param['pg_id'] = $obj->pgId?trim($obj->pgId):'';
			$param['session'] = $obj->txnId?trim($obj->txnId):'';
			$param['payamnt'] = $obj->amount?trim($obj->amount):'';
			$param['emailid'] = $obj->emailId?trim($obj->emailId):'';
			$param['mobileno'] = $obj->mobileNo?trim($obj->mobileNo):'';
			$param['can_id'] = $obj->canId?trim($obj->canId):'';
			$param['sub_segment'] = $obj->requestType?trim($obj->requestType):'';

                        if(!empty($param['pg_id']) && !empty($param['session']) && !empty($param['payamnt']) && !empty($param['emailid']) && !empty($param['mobileno']) && !empty($param['can_id']) && !empty($param['sub_segment'])){
                                $data = createTransaction($param);
                        }else{
                                $data = array("status"=>"failure","response"=>array(), "message"=>"Invalid request format");
                        }
                break;

		case 'responsePayment':
#                       print_r($obj); exit;
                        $order_id = $obj->orderId?trim($obj->orderId):'';
                        if(!empty($order_id)){
                                $data = responsePayment($order_id);
                        }else{
                                $data = array("status"=>"failure","response"=>array(), "message"=>"Invalid request format");
                        }
                break;
...
/********** Start Autopay SDK Integration API ********/
		case 'createOrder':
#                       print_r($obj); exit;
                        $param['name'] = $obj->custName?trim($obj->custName):'';
                        $param['amount'] = $obj->amount?trim($obj->amount):'';
                        $param['emailid'] = $obj->emailId?trim($obj->emailId):'';
                        $param['mobileno'] = $obj->mobileNo?trim($obj->mobileNo):'';
                        $param['can_id'] = $obj->canId?trim($obj->canId):'';
                        $param['request_type'] = $obj->requestType?trim($obj->requestType):'';

                        if(!empty($param['name']) && !empty($param['amount']) && !empty($param['emailid']) && !empty($param['mobileno']) && !empty($param['can_id']) && !empty($param['request_type'])){
                                $data = createOrder($param);
                        }else{
                                $data = array("status"=>"failure","response"=>array(), "message"=>"Invalid request format");
                        }
                break;

		case 'responsePaymentForAutopay':
#                       print_r($obj); exit;
                        $order_id = $obj->orderId?trim($obj->orderId):'';
			$payment_id = $obj->paymentId?trim($obj->paymentId):'';
                        if(!empty($order_id) && !empty($payment_id)){
                                $data = responsePaymentForAutopay($order_id, $payment_id);
                        }else{
                                $data = array("status"=>"failure","response"=>array(), "message"=>"Invalid request format");
                        }
                break;
...
/************ Start for MGM *******/
		...
		case 'createLeadForMGM':
			$param = array(
			"USERNAME"=>$obj->pushData->USERNAME,
			"PASSWORD"=>$obj->pushData->PASSWORD,
			"ReferalAccountID"=>$obj->pushData->ReferalAccountID,
			"Topic"=>$obj->pushData->Topic,
			"Segment"=>$obj->pushData->Segment,
			"Sub_BusinessSegment"=>$obj->pushData->Sub_BusinessSegment,
			"Email_Id"=>$obj->pushData->Email_Id,
			"ContactPersonName"=>$obj->pushData->ContactPersonName,
			"SalutationID"=>$obj->pushData->SalutationID,
			"Mobile_No"=>$obj->pushData->Mobile_No,
			"Country"=>$obj->pushData->Country,
			"City"=>$obj->pushData->City,
			"State"=>$obj->pushData->State,
			"Area"=>$obj->pushData->Area,
			"FormName"=>$obj->pushData->FormName,
			"PostalCode"=>$obj->pushData->PostalCode,
			"houseflatno"=>$obj->pushData->houseflatno,
			"Lead_First_Name"=>$obj->pushData->Lead_First_Name,
			"Lead_Last_Name"=>$obj->pushData->Lead_Last_Name,
			"SocietyBuidling"=>$obj->pushData->SocietyBuidling,
			"IPAddress"=>$obj->pushData->IPAddress,
			"CreatedOnDate"=>$obj->pushData->CreatedOnDate
			);
                        if(!empty($param)){
                                $data = createLeadForMGM($param);
                        }else{
                                $data = array("status"=>"failure","response"=>array(), "message"=>"Invalid request format");
                        }
                break;
...
/**************** Create/Update Lead *****************/
		case 'createLead':
		$client_id = $obj->client_id?trim($obj->client_id):'';
        	$first_name = $obj->first_name?trim($obj->first_name):'';
        	$last_name = $obj->last_name?trim($obj->last_name):'';
		$mobile = $obj->mobile?trim($obj->mobile):'';
		$email = $obj->email?trim($obj->email):'';
		$society_id = $obj->society_id?trim($obj->society_id):'';
		$identifier = $obj->identifier?trim($obj->identifier):'';
		$crm_state_id = $obj->crm_state_id?trim($obj->crm_state_id):'';
		$crm_city_id = $obj->crm_city_id?trim($obj->crm_city_id):'';
		$crm_city_no = $obj->crm_city_no?trim($obj->crm_city_no):'';
		$crm_area_id = $obj->crm_area_id?trim($obj->crm_area_id):'';
		$society_name = $obj->society_name?trim($obj->society_name):'';
		$area_name = $obj->area_name?trim($obj->area_name):'';
		$city_name = $obj->city_name?trim($obj->city_name):'';
		$building_status = $obj->building_status?trim($obj->building_status):'';
		$productcode = $obj->productcode?trim($obj->productcode):'';
		$quantity = $obj->quantity?trim($obj->quantity):'';
	        $non_recommend_productcode = $obj->non_recommend_productcode?trim($obj->non_recommend_productcode):'';
        	$non_recommend_quantity = $obj->non_recommend_quantity?trim($obj->non_recommend_quantity):'';
		$address = $obj->address?trim($obj->address):'';
		$utm_source = $obj->utm_source?trim($obj->utm_source):'';
		$utm_medium = $obj->utm_medium?trim($obj->utm_medium):'';
		$utm_campaign = $obj->utm_campaign?trim($obj->utm_campaign):'';
		$utm_term = $obj->utm_term?trim($obj->utm_term):'';
		$utm_content = $obj->utm_content?trim($obj->utm_content):'';
		
	 		
		$req_data = array("secret_key" => "6aVVp6gZbzuySeGz8EAO3qKR04klEizE56" ,"client_id"=>$client_id, "first_name"=>$first_name, "last_name"=>$last_name,"mobile"=>$mobile, "email"=>$email,"society_id"=>$society_id,"identifier"=>$identifier,"crm_state_id"=>$crm_state_id,"crm_city_id"=>$crm_city_id,"crm_city_no"=>$crm_city_no,"crm_area_id"=>$crm_area_id,"society_name"=>$society_name,"area_name"=>$area_name,"city_name"=>$city_name,"building_status"=>$building_status, "productcode"=>$productcode, "quantity"=>$quantity, "non_recommend_productcode"=>$non_recommend_productcode, "non_recommend_quantity"=>$non_recommend_quantity, "address"=>$address,"utm_source"=>$utm_source,"utm_medium"=>$utm_medium,"utm_campaign"=>$utm_campaign,"utm_term"=>$utm_term, "utm_content"=>$utm_content);
	
		$data = CreateLead($req_data);
		break;
		...
		$client_id = $obj->client_id?trim($obj->client_id):'';
	        $first_name = $obj->first_name?trim($obj->first_name):'';
        	$last_name = $obj->last_name?trim($obj->last_name):'';
	        $lampleadid = $obj->lampleadid?trim($obj->lampleadid):'';
        	$productcode = $obj->productcode?trim($obj->productcode):'';
        	$quantity = $obj->quantity?trim($obj->quantity):'';
	        $non_recommend_productcode = $obj->non_recommend_productcode?trim($obj->non_recommend_productcode):'';
        	$non_recommend_quantity = $obj->non_recommend_quantity?trim($obj->non_recommend_quantity):'';
		...
		$req_data = array("secret_key" => "6aVVp6gZbzuySeGz8EAO3qKR04klEizE56" ,"client_id"=>$client_id, "first_name"=>$first_name, "last_name"=>$last_name,"lampleadid"=>$lampleadid, "productcode"=>$productcode, "quantity"=>$quantity, "non_recommend_productcode"=>$non_recommend_productcode, "non_recommend_quantity"=>$non_recommend_quantity);
#		print_r($req_data); exit;	
		$data = UpdateLead($req_data);
...
/*************Start CRM website **********/
	   case 'trustBandCustomerCount':
                $param['SocietyId'] = $obj->SocietyId?trim($obj->SocietyId):'';
                $param['BuildingId'] = $obj->BuildingId?trim($obj->BuildingId):'';
             	
		$data = trustBandCustomerCount($param);
		break;
