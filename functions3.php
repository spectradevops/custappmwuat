...
function comparisonPlan($param){
	 ...
	 $db = dbConnection();
	 $query = "select rateplan_id, description, data, base_price, download, month, duration, extra_month from scp_topup_plan.plan_mstr_web where rateplan_id in (".$planid.")"; 
...
#		print_r($getcust); exit; 		
		...
		$query = "select base_price, duration from scp_topup_plan_uat.plan_mstr_web where rateplan_id = '".$cur_plan_id."'";
                $stmt = $db->prepare($query);
                $stmt->execute();
                $row = $stmt->fetchAll(PDO::FETCH_ASSOC);
                if(!empty($row)){
			...
			return $response;
         	...
         	$query = "select base_price, duration from scp_topup_plan_uat.plan_mstr_web where rateplan_id = '".$plan_id."'";
         	$stmt = $db->prepare($query);
         	$stmt->execute();
         	$row = $stmt->fetchAll(PDO::FETCH_ASSOC);
         	if(!empty($row)){
		...
		$result = array("refundAgainstOldPlan"=> "-".$oldPlanReversal, "newPlanBillAmount" => $newPlanBillAmount, "proDataCharges" => $totalPayableAmount, "pgDataCharges" => $pgDataCharges); 
...
function proDataChargesForTopup($can_id, $topup_id){
		...
		$db = dbConnection();
        	$query = "select price, type from scp_topup_plan_uat.topup_mstr_web where topup_id = '".$topup_id."'";
         	$stmt = $db->prepare($query);
         	$stmt->execute();
         	$row = $stmt->fetchAll(PDO::FETCH_ASSOC);
         	if(!empty($row)){
