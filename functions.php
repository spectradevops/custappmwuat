...
return $data;
	...
	function getCustomerAccountDetail1($can_id){
		
#	exit;	

		$getData=getCustomerAccountDetail11($can_id);
		//print_r($getData);
			if(empty($getData)){
					$data = array("status"=>"failure","response"=>array(),"message"=>"No Deatils found.");
			}else{
					$data = array("status"=>"success","response"=>$getData, "message"=>"Successfully Fetched");
			}
	return $data;
		...
		if(!empty($start_date) && !empty($end_date)){
			...
			#print_r($dataArr); exit;
					...
					//echo $st_date." ".$in_date." ".$ed_date;exit;
						...
						if($in_date > 20210401){
                                                $value['displayInvNo'] = substr($value['cslno'],4,-6);
                                                }else{
...
#		print_r($response['Message']); exit;
		...
		foreach($response as $key =>$val){
			...
			$getSR[$i]['createdon'] = $val['createdon'];
			$getSR[$i]['Owner'] = $val['Owner'];
			$getSR[$i]['RC1'] = $val['closurecategoryRC1'];
			$getSR[$i]['RC2'] = $val['closureSubCategoryRC2'];
			$getSR[$i]['RC3'] = $val['closureSubSubCategoryRC3'];
...
#		print_r($getContact); exit;
		switch($getContact['contactTypeNo']){
			...
			$BillingTo['address']      = $getContact['street'].", ".$getContact['pin'];
                        $getCommM             = getContactCommMedium($getContact['contactNo']);
			#print_r($getCommM); exit;
                        if(!empty($getCommM)){
			  $CountArr   =  count(array_column($getCommM, 'commMediumNo'));
	                   if($CountArr == 0){
        	                   $getCommMM[0]    = $getCommM;
                	   }else{
                        	$getCommMM      = $getCommM;
                   	}

                                foreach($getCommMM as $getComm){
                                        switch($getComm['commTypeNo']){
                                                case 2:
...
/********************** Adding default value for Account Manager 16 Dec'21 *****************/
		#	if(empty($BillingTo)){
				$BillingTo['name']      =  !empty($BillingTo['name'])?$BillingTo['name']:'Spectra Support';
				$BillingTo['address']      =  !empty($BillingTo['address'])?$BillingTo['address']:'address';
				$BillingTo['mobile']      =  !empty($BillingTo['mobile'])?$BillingTo['mobile']:'1800 121 5678';
				$BillingTo['email']      =  !empty($BillingTo['email'])?$BillingTo['email']:'support@spectra.co';
		#	}
/*********************** End **********************/
			$getProfile = array("billTo" => $BillTo, "shipTo"=> $ShipTo, "accountManager" => $BillingTo, "installationA" =>$InstATo, "installationB" =>$InstBTo, "GSTN" =>$gstn, "TAN" =>$tan);
...
#               	print_r($getTopupList); exit;
			...
			$closedate = date("Ymd",strtotime($getTopup['closeDate']));
			if($closedate < date("Ymd")){
...
#		$newPlanBillAmount = $newBasePrice + $newPlanCharges;
		...
		$remarks = "If there is any balance amount, this balance amount will be adjusted in your next invoice"; // Added by Sunit

		$result = array("Current plan Activation date" => date("d/m/Y"), "Charges for Duration consumed" => $oldPlanCharges, "Balance amount to be reversed"=> $oldPlanReversal, "New Plan Charges" => $newPlanBillAmount, "Difference Amount Payable" => $totalPayableAmount, "taxes" => $taxes, "pgDataCharges" => $pgDataCharges, "remarks" => $remarks); 
#print_r($result);
...
function knowMoreForPlan($plan_id){
		...
		$db = scp_topup_plan_db();
                $query = "select * from know_more_plan where plan_id = '".$plan_id."'";
                $stmt = $db->prepare($query);
                $stmt->execute();
                $row = $stmt->fetchAll(PDO::FETCH_ASSOC);
                if(!empty($row)){
		...
		$planDescription = $row[0]['plan_desc'];
	
	$contentText[] =  array("iconId" => $row[0]['icon_id1'],
                                        "title" => $row[0]['title1'],
                                        "content" => $row[0]['content1']
                        	);
	$contentText[] =  array("iconId" => $row[0]['icon_id2'],
                                        "title" => $row[0]['title2'],
                                        "content" => $row[0]['content2']
                                );
	$contentText[] =  array("iconId" => $row[0]['icon_id3'],
                                        "title" => $row[0]['title3'],
                                        "content" => $row[0]['content3']
                                );
        $contentText[] =  array("iconId" => $row[0]['icon_id4'],
                                        "title" => $row[0]['title4'],
                                        "content" => $row[0]['content4']
                                );
 	  $contentText[] =  array("iconId" => $row[0]['icon_id5'],
                                        "title" => $row[0]['title5'],
                                        "content" => $row[0]['content5']
                                );
	...
	$response = array("status"=>"success",
		...
		$response = array("status"=>"failure", "response"=> array(),"message"=>"Oops! Something has gone wrong.");
...
#		print_r($param); exit; 
		...
		for($i = 0; $i < $count; $i++){
			...
			$token = $param["token"][$i];
			$type  = $param["type"][$i];
			if(!empty($canid) && !empty($token) && !empty($type)){
			$sql = "select * from device_token_master where device_token = '".$token."' and can_id ='".$canid."'";
			$stmt = $db->prepare($sql);
	                $stmt->execute();
                	$row = $stmt->fetchAll(PDO::FETCH_ASSOC);
			if(empty($row[0]['device_token'])){
...
#               print_r($param); exit; 
                ...
                for($i = 0; $i < $count; $i++){
                        ...
                        $row = $stmt->fetchAll(PDO::FETCH_ASSOC);
                        if(!empty($row[0]['device_token'])){
                        $query = "delete from device_token_master where device_token = '".$token."' and can_id ='".$canid."'";
                        $stmt = $db->prepare($query);
                        $stmt->execute();
                        }
                }// End of Loop
...
function pushNotification($title, $short_description, $detailed_description, $canids, $type){
	$db = dbConnection();
	foreach($canids as $canid){
		...
		$query = "insert into push_notification (canid, type, title, short_description, detailed_description, create_date) values ('".$canid."', '".$type."', '".$title."','".$short_description."','".$detailed_description."', now())";

		$stmt = $db->prepare($query);
                $stmt->execute();
		$id = $db->lastInsertId();
		$data = array(	"can_id" => $canid, 
			      	"image_url" => "string", 
				"pdf_url" => "string", 
		...
		$return = dynamicNotification($data);
		$query = "update push_notification set flag = 'true' where id = '$id'";
                $stmt = $db->prepare($query);
                $stmt->execute();
	
	}
	$response = array("status"=>"success","response"=> $return,"message"=>"Notification has been pushed successfully");
	return $response;
}

function getDeviceToken($canids){
#               print_r($param); exit; 
                $db = dbConnection();
                $count = count($canids);
		$result = array();
		...
		$result[] = array("canId" => $canid, "deviceToken" => $device_token);	
                }// End of Loop
                $response = array("status"=>"success","response"=> $result,"message"=>"Successfully fetched");
...
function sendPaymentLink($params){
	...
	$CANID = $params["canId"];
	$txnid = date("YmdHis").rand(1001,9999);
	$mobile_number = $params["mobile"];
	$email_id    = $params["email"];
	$outstandingBalance = $params["amount"];
#	$dueDate = $params["DueDate"];
	$account_name = $params["act_name"];
	$segment = $params["segment"];

	$encryptedString =EncryptLink($CANID, "1234"); //Generate EncryptLink for Payment
        $encryptedString = urlencode($encryptedString);
        $linkValidTime = date('Y-m-d H:i:s',strtotime("1 Day"));

	$db  = dbConnection();
	$sql = "insert into ai_db.custPaymentLinkInfo (can_id, txnid, account_name, email, mobile, balance_amount, payment_link, link_valid_time, segment, status, created_date) values('".$CANID."','".$txnid."','".$account_name."','".$email_id."','".$mobile_number."','".$outstandingBalance."','".$encryptedString."','".$linkValidTime."','".$segment."','pending',now())";
	LogRequest($sql,"common_file.log");
...
//Send Email
	$subject  = "Payment Link of Customer Account Number: ".$CANID;

	$content = "Dear Customer,<br/><br/>The outstanding for Customer Account Number ".$CANID." is Rs.".$outstandingBalance.". You can click 
	<a href='https://epay.spectra.co/inw/easyPay/?id=".$encryptedString."'>here</a> to make the payment.
	<br/><br/>
	You can view or download your invoices and make online payments 
	from <a href='https://my.spectra.co'>https://my.spectra.co</a>
	<br><br>
	Regards,
	<br>
	Team Spectra";

  	$from = "donotreply@spectra.co";
        ...
        $mail_data['bcc'] = "";
    	$mail_data['from'] = $from;
    	$mail_data['email_id'] = $email_id;
	$email_response= SendEmail($mail_data) ;
	$email_status =$email_response['status'];

	$Logtxt = $email_id."\t".$email_response['message'];
	LogRequest($Logtxt,"common_file.log");

	$sms_link_org = "https://epay.spectra.co/inw/easyPay/?id=".$encryptedString;
	$sms_link = shortenURL($sms_link_org);
	$mbl_msg  = "Dear Customer, \nThe outstanding for Customer Account Number ".$CANID." is	Rs.".$outstandingBalance.". You can click $sms_link to make the payment. \nTeam Spectra";

	$sms_response= SendSMS($mobile_number,$mbl_msg) ;
	$sms_status =$sms_response['status'];
	
	$Logtxt = $mobile_number."\t".$sms_response['message'];
        LogRequest($Logtxt,"common_file.log");
	if($email_status == "success" && $sms_status == "success"){
	 	$data = array("status"=>"success","link"=>$sms_link, "id"=>$insert_id, "message"=>"Payment link sent successfully");
	}else{
		$data = array("status"=>"failure", "message"=>"Payment link could not send");
	}
	return $data;
}

function LogRequest($request,$file=null)
{
	$t = explode(" ",microtime());
	$timestamp = date("Y-m-d H:i:s",$t[1]).(string)$t[0];
	$LOG = $timestamp." ".$request;
	if($file=='')   $file="common.log";
	file_put_contents("logs/".$file,"\n".$LOG, FILE_APPEND | LOCK_EX);
}

function sendPaymentLinkForAirmesh($param){
	$db  = dbConnection();
        $sql = "insert into airmesh_payment_link (can_id,cust_name,industry_segment,product_segment,sr_no,sr_creation_date,topup_id,topup_mode,total_amount,router,req_date,created_by,mobile,email_id,install_city,link_send_status) values('".$param['can_id']."','".$param['cust_name']."','".$param['industry_segment']."','".$param['product_segment']."','".$param['sr_number']."','".$param['sr_creation_date']."','".$param['topup_id']."','".$param['topup_mode']."','".$param['topup_amount']."','".$param['router_detail']."',now(),'".$param['created_by']."','".$param['mobile']."', '".$param['email']."','".$param['inst_city']."', 'pending')";
        LogRequest($sql,"common_file.log");
        $stmt = $db->prepare($sql);
        $stmt->execute(); 
	$id = $db->lastInsertId();
						$params['canId']  = $param['can_id'];
	                         		$params['amount'] = $param['topup_amount'];
                                                $params['email']  = $param['email'];
                                                $params['mobile'] = $param['mobile'];
                                                $params['act_name'] = $param['cust_name'];
                                                $params['segment'] = 'Home';
                                                $result = sendPaymentLink($params);
#print_r($result); exit;	
	if($result['status'] == "success"){
 		$update = "update airmesh_payment_link set link_id='".$result['id']."', payment_link='".$result['link']."', link_send_status='sent', link_send_date=now()  where id ='$id'";
...
/********************Start PG SDK integration ************/
require('razorpay-php/Razorpay.php');
use Razorpay\Api\Api;

function createTransaction($param){
		if(strtoupper($param['pg_id']) == "RAZORPAY"){
			$keyId = 'rzp_test_F1AUKPFJBtNA1v';// UAT FTTH Key
			$keySecret = 'jGhgyxcd3vW8VQuzYLVdRkhB'; // UAT FTTH secret

#			$keyId = 'rzp_live_shczEZGTiK9r5L'; // Live FTTH key  
#			$keySecret = 'PW1pX25ngmcx4GqiWOsQpoq6'; // Live FTTH secret
			$UserID_DBsave = "sunit.sinha@spectranet.in"; // Only to store in DB
			...
			$orderData = array('receipt' => $param['session'],
                        'amount'          => $param['payamnt'] * 100, //  rupees in paise
                        'currency'        => 'INR',
			...
			$sql = "insert into payment_api (txnid,amount,req_date,segment,sub_segment,paytype,emailid,mobileno,source,source_ip,p_gateway, can_id, order_id) values ('".$param['session']."','".$param['payamnt']."',now(),'$segment','".$param['sub_segment']."','$paytype','".$param['emailid']."','".$param['mobileno']."','$source','$source_ip','".$param['pg_id']."','".$param['can_id']."','".$razorpayOrderId."')";
			$stmt = $db->prepare($sql);
	                $stmt->execute();
			$last_id = $db->lastInsertId();
	                if($last_id > 0){
        	                $response = array("status"=>"success","response"=> array("id"=>$last_id, "txnId"=> $param['session'], "keyId" => $keyId, "keySecret" => $keySecret, "pgId" => $param['pg_id'], "orderId" => $razorpayOrderId), "message"=>"Transaction added successfully");
                	}else{
                        	$response = array("status"=>"failure","response"=> array(), "message"=>"Oops! something went wrong");
                	}
		}else{
			$response = array("status"=>"failure","response"=> array(), "message"=>"Invalid format");
		}
return $response;
}

function responsePayment($order_id){
	 	$db  = epay_db();
		$sql = "select txnid, order_id, amount, mobileno from payment_api where order_id ='$order_id'";
		$stmt = $db->prepare($sql);
	        $stmt->execute();
        	$row = $stmt->fetchAll(PDO::FETCH_ASSOC);		
...
#                       $keySecret = 'PW1pX25ngmcx4GqiWOsQpoq6'; // Live FTTH secret
			...
			$record = count($items);
		        ...
		        for($j = $record ; $j > 0; $j--){
		        $i = $j - 1;
        		$paid_amount = $items[$i]->amount;
		        $paid_amount = $paid_amount/100;
		        $pgTxnNo     = $items[$i]->id;
		        $successTxn  =  $items[$i]->status;
		        $paymentmode = $items[$i]->method;
		        $txn_id       = $items[$i]->notes->merchant_order_id;
		        $captured    = $items[$i]->captured;
		        $error_description = $items[$i]->error_description;
		        if($captured == "1"){
                	$pgrespcode = 0;
                	...
                	$pgrespcode = 1;
	                $txnmsg = $error_description;
        	        $txnstatus = "FAIL";
			 }else{
	                $pgrespcode = "11";
        	        $txnmsg = $successTxn;
                	$txnstatus = $successTxn;
        		}

		        if($successTxn == "refunded"){
                	$pgrespcode = 15;
	                $txnmsg = "Transaction Refunded";
        	        $txnstatus = "refunded";
		        }
			$sql_upd = "update payment_api set paid_amount='$paid_amount', pgTxnNo='$pgTxnNo', pgRespCode='$pgrespcode', TxMsg='$txnmsg', successTxn='$txnstatus', paymentmode = '$paymentmode', res_date=now(), recons='1' where order_id = '$order_id'";
			$stmt = $db->prepare($sql_upd);
                        $stmt->execute();
                        $message = $stmt->rowCount() . " record UPDATED successfully";
                        $response = array("status"=>"success","response"=> array("orderId" => $order_id, "paymentId" => $pgTxnNo, "paymentStatus" => $txnstatus), "message"=>$message);
			if($pgrespcode == 0){ break; }
			}// End of If
		        }// End of For Loop
		}catch(Exception $e) {
			  $error =  'Message: ' .$e->getMessage();
			$response = array("status"=>"failure","response"=> array(), "message"=>$error);
 		}
		}else{
                        $response = array("status"=>"failure","response"=> array(), "message"=>"Order Id not valid.");
                }
...
/********************Start PG SDK integration for AutoPay ************/
#require('razorpay-php/Razorpay.php');
#use Razorpay\Api\Api;

 function createOrder($param){
                if(!empty($param['can_id'])){
		$db  = epay_db();
			...
			$razorpayOrder = $api->order->create($orderData);
			$last_id =  insert_ftthpaymentsi($param,$param['amount'],$razorpayOrder,$customerid,$source,$source_ip,$return_url); //insert in DB
	               $razorpayOrderId = $razorpayOrder['id'];
 #        	       $_SESSION['razorpay_order_id'] = $razorpayOrderId;


                        if($last_id > 0){
                                $response = array("status"=>"success","response"=> array("id"=>$last_id, "orderId"=> $razorpayOrderId, "customerId" => $customerid, "canId" => $param['can_id'], "paidAmount" => $param['amount'], "keyId" => $keyId, "keySecret" => $keySecret), "message"=>"Transaction added successfully");
                        }else{
                                $response = array("status"=>"failure","response"=> array(), "message"=>"Oops! something went wrong");
                        }
                }else{
...
function responsePaymentForAutopay($order_id, $payment_id){
	$db  = epay_db();
	$query = "select * from razorpay_si where order_id ='".$order_id."'";
	$stmt = $db->prepare($query);
        $stmt->execute();
	$row = $stmt->fetchAll(PDO::FETCH_ASSOC);
	if(!empty($row[0]['order_id'])){
	$razorpay_si_id = $row[0]['id'];
	$canid 		= $row[0]['canid'];
	$customer_id 	= $row[0]['customer_id']; 
 	$email   	= $row[0]['email'];
	$contact 	= $row[0]['mobileno'];
#$keyId = 'rzp_live_OXG4iUDctFasFu'; // For Live
#$keySecret = 'bIyXd99g2gCTWhrAd8CdeV6O'; // For Live
$keyId='rzp_test_zrullBk9rAHd75'; // For UAT
$keySecret='sOf42jZd2aSZrxLvHF1ENyFe'; // For UAT
	...
	$api = new Api($keyId, $keySecret);
			$obj = $api->payment->fetch($payment_id);/*Get token id and all detail*/
#	print_r($payment);exit;
	...
	if($last_id > 0){
			if($status == "captured"){
			$status='1';
                        $sql = "UPDATE razorpay_si SET si_status=? WHERE  id=?";
                        $arr=array($status,$razorpay_si_id);
                        $res=$db->prepare($sql)->execute($arr);
			$message = $stmt->rowCount() . " record UPDATED successfully";
                        $response = array("status"=>"success","response"=> array("id" => $last_id, "orderId" => $order_id, "paymentId" => $payment_id, "paymentStatus" => "SUCCESS" ), "message"=>$message);
			return $response;
			}else{
			$message = "Autopay could not enabled";
			$response = array("status"=>"success","response"=> array("id" => $last_id, "orderId" => $order_id, "paymentId" => $payment_id, "paymentStatus" => "Fail" ), "message"=>$message);
			}
		}else{
			$response = array("status"=>"failure","response"=> array(), "message"=>"Oops! something went wrong");		
		}
...
function disableOrder($can_id){
	$db = epay_db();
	$status='0';
                        $sql = "UPDATE razorpay_si SET si_status=? WHERE  canid=?";
                        $arr=array($status,$can_id);
                        $res=$db->prepare($sql)->execute($arr);
                        $message = "Disabled successfully";
                        $response = array("status"=>"success","response"=> array("canId" => $can_id), "message"=>$message);
                        return $response;
...
function createLeadForMGM($data){
#print_r($data); exit; 
if(empty($data)){
		return false;
	}
	...
	try {
	    $options = array(
            	...
            	$client = new SoapClient(CRM_LEAD, $options);
	    	$response = $client->__soapCall('CreateLeadForMGM', array($data));
		$log_txt = "\n".date("Y-m-d H:i:s")."\n".$client->__getLastRequest();
                $log_txt .= "\n".date("Y-m-d H:i:s")."\n".$client->__getLastResponse();
                file_put_contents("logs/mgm_lead.log","\n".$log_txt, FILE_APPEND | LOCK_EX);
           	$crm_status['crm_result'] = $response->CreateLeadForMGMResult->Leadid;
                $crm_status['status']=$response->CreateLeadForMGMResult->status;
	    } catch (SoapFault $e) {
                $log_txt = "\n".date("Y-m-d H:i:s")."\n".$e;
...
function getAccountStatus($can_id){

	$data = getCustomerAccountDetail11($can_id);
	if(isset($data['accountId']) && $data['accountId'] == $can_id){
		$result = array("status"=>"success","response"=>array("status"=>"Active", "name"=>$data['accountName'], "mobile" => $data['mobileno'], "email"=>$data['email']),"message"=>"Account is active");
	}else{
		$result = array("status"=>"success","response"=>array("status"=>"Inactive"),"message"=>"Account is inactive");
