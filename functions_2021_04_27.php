...
function updatePaymentStatus($pg_id, $id, $order_id, $pgTxnNo, $pgRespCode, $successTxn, $TxMsg, $paid_amount, $paymentmode){
if(strtoupper($pg_id) == "RAZORPAY"){
	 ...
	 if($pgRespCode == "1"){
                ...
                $success = false;
                        ...
                        $db  = epay_db();
			$update = "update payment_api set paid_amount='$paid_amount', pgTxnNo='$pgTxnNo', pgRespCode='$pgrespcode', TxMsg='$txnmsg', successTxn='$txnstatus', paymentmode = '$paymentmode', res_date=now() where txnid = '".$id."'" ;
                        $stmt = $db->prepare($update);
                        $stmt->execute();
                        $message = $stmt->rowCount() . " record UPDATED successfully";
                        $response = array("status"=>"success","response"=> array("id" => $id, "orderId" => $order_id, "paymentId" => $pgTxnNo), "message"=>$message);
...
function updateStatusForAutopay($param){
	$db  = epay_db();
	$query = "select * from razorpay_si where order_id ='".$param['order_id']."'";
	$stmt = $db->prepare($query);
        $stmt->execute();
	$row = $stmt->fetchAll(PDO::FETCH_ASSOC);
	if($row[0]['si_status'] == 1 ){
		$response = array("status"=>"failure","response"=> array(), "message"=>"Autopay already enabled.");
	}elseif(!empty($row[0]['order_id'])){
	...
	if($last_id > 0){
			...
			$message = $stmt->rowCount() . " record UPDATED successfully";
                        $response = array("status"=>"success","response"=> array("id" => $last_id, "orderId" => $order_id, "paymentId" => $payment_id), "message"=>$message);
			return $response;
		}else{
			$response = array("status"=>"failure","response"=> array(), "message"=>"Oops! something went wrong");			
		}
